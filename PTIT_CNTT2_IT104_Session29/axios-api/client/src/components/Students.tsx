...
export default function Students() {
  ...
  const getAllStudent = async () => {
    try {
      const res = await axiosClient.get<StudentType[]>("/student");
  ...
  const getStudentById = async (id: number) => {
    try {
      const res = await axiosClient.get<StudentType>(`/student/${id}`);
      if (res.data) {
  ...
  const createStudent = async (student: {
    ...
    try {
      const res = await axiosClient.post<StudentType>("/student", student);
  ...
  useEffect(() => {
    getAllStudent();
    const newStudent = {
      student_name: "Tuan Anh",
      email: "t.anh@gmail.com",
      address: "Lang Son",
      phone: "0912345678",
      status: true,
