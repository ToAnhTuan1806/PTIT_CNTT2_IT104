...
class Vehicle {
    ...
    basePrice;
    status;
    constructor(vehicleId, brand, model, basePrice, status = "Sẵn sàng") {
...
class RentalContract {
    ...
    days;
    totalCost;
    constructor(contractId, customer, vehicle, days) {
...
class RentalSystemManager {
    ...
    contractCounter;
    constructor() {
        this.customersRepo = new Repository();
        this.vehiclesRepo = new Repository();
        this.contractsRepo = new Repository();
        this.contractCounter = 1;
    }
    addCustomer(id, name, phone, license) {
        let cus = new Customer(id, name, phone, license);
        this.customersRepo.add(cus);
        console.log("Them KH thanh cong");
    }
    addVehicle(type, id, brand, model, price) {
        let v;
        if (type === "motorbike") {
    ...
    createContract(customerId, vehicleId, days) {
    ...
    listContractsByCustomer(customerId) {
...
function startMenu() {
    ...
    let choice;
    do {
        choice = prompt("===== MENU QUẢN LÝ HỆ THỐNG THUÊ XE =====\n" +
            "1. Thêm khách hàng\n" +
            "2. Thêm xe\n" +
            "3. Tạo hợp đồng thuê\n" +
